...
class TestStore(testing.TestCase):
    ...
    def test_store_not_found(self):
        r = self.simulate_get('/store/test', headers=self.access_token)
    ...
    def test_store_found(self):
        ...
        self.sess.add(store)
        self.sess.commit()
        r = self.simulate_get('/store/testStore', headers=self.access_token)
    ...
    def test_delete_store(self):
        ...
        self.sess.add(store)
        self.sess.commit()
        r = self.simulate_delete('/store/testStore', headers=self.access_token)
        self.assertEqual(r.status_code, 200)
        self.assertDictEqual({'message':'Store deleted'}, r.json)

    def test_create_store(self):
        r = self.simulate_post('/store/test', headers=self.access_token)
        self.assertEqual(r.status_code, 201)
        self.sess.query(StoreModel).filter(StoreModel.name=='test').first()
        self.assertDictEqual({'name':'test', 'items':[]}, r.json)

    def test_create_duplicate_store(self):
        self.simulate_post('/store/test', headers=self.access_token)
        r = self.simulate_post('/store/test', headers=self.access_token)
    ...
    def test_list_store(self):
        ...
        self.sess.add(store)
        self.sess.commit()
        r = self.simulate_get('/stores', headers=self.access_token)
